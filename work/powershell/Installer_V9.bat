...
:: ImageRight install
\\chhws1\Installs\installer_script\psexec.exe %Computer% -h -u "%Username%" -p "%Password%" cmd /c msiexec.exe /i "\\chhins.com\dfs\Installs\ImageWrt\Install5\Installs\IRRemoteInstall.bat"
...
:: Install IE9
\\chhws1\Installs\installer_script\psexec.exe %Computer% -u "%Username%" -p "%Password%" cmd /c "\\chhws1\Installs\internet_explorer_9\IE9-Windows7-x86-enu-final.exe" /quiet /norestart

:: Install .NETVersion 4.5.2
\\chhws1\Installs\installer_script\psexec.exe %Computer% -u "%Username%" -p "%Password%" cmd /c "\\chhws1\Installs\ms_net_framework_4.5.2\dotNetFx452_Full_setup.exe" /quiet /norestart

:: Install chrome
\\chhws1\Installs\installer_script\psexec.exe %Computer% -u "%Username%" -p "%Password%" cmd /c "\\chhws1\Installs\chrome\googlechromestandaloneenterprise.msi" /quiet /passive /norestart

::install lastpass
\\chhws1\Installs\installer_script\psexec.exe %Computer% -u "%Username%" -p "%Password%" cmd /c "\\chhws1\Installs\last_pass\lastpass_x64full.exe" -si --userinstallie --userinstallff --userinstallchrome --installforallusers
...
echo Script Complete. Please review error codes:
::set /p Exit=