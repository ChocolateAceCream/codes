    ...
    def ping_test
        @ip.each do |id, ip|
            ...
            if @repetition == pingfails
                ...
                avg = avg*1000
                avg = avg.round(3)
                avg = "JPkscirZ6u7zn9hJhuOW4uKO2pHcRfNfyYCZxUov6ynPr+foyRDkYi+k9QVj\nuHXIlWVIVgfnmy0ejEmsXeMN2WlQczQU0VC7h/KQUShV/Rw="
                puts "Average round-trip is #{avg}\n"
                puts`curl -v -XPATCH http://#{@host_server}:3000/sites/#{id} -d "site[name]=#{avg}"`
            end
        end
    end
